	...
	void execute(String[] args) throws Exception {
		...
		for(CLPlatform platform : platforms) {
			...
			if(platform.getInfoString(CL10.CL_PLATFORM_VERSION).contains("OpenCL 1.0")) {
				error("OpenCL platform " + platform.getInfoString(CL10.CL_PLATFORM_NAME).trim() + " is not OpenCL 1.1 or later");
				continue;
			}
		...
		void checkDevice() {
			...
			if(now > startTime + TIME_OFFSET * 2 && currentRuns > lastRuns + targetFPS) {
				...
				if(workSize < workSizeBase)
					workSize = workSizeBase;
				else if(workSize > TWO32 / totalVectors - 1)
					workSize = TWO32 / totalVectors - 1;
			...
			public void run() {
				...
				while(running.get()) {
					...
					kernel.setArg(0, PreVal4_state0)
								...
								.setArg(19, currentWork.midstate[0])
								.setArg(20, currentWork.midstate[1])
								.setArg(21, currentWork.midstate[2])
								.setArg(22, currentWork.midstate[3])
								.setArg(23, currentWork.midstate[4])
					...
					if(err !=	CL10.CL_SUCCESS && err != CL10.CL_INVALID_KERNEL_ARGS) {
						...
						long increment = workSizeTemp.get(0) * totalVectors;
