...
__kernel __attribute__((vec_type_hint(u))) WORKGROUPSIZE void search0(
...
__kernel __attribute__((vec_type_hint(u))) WORKGROUPSIZE void search1(
    const uint block0, const uint block1, const uint block2,
    const uint state0, const uint state4,
    const uint B1, const uint C1, const uint D1,
    const uint F1, const uint G1, const uint H1,
    const uint base,
    __global u * store0, __global u * store1,
    __global u * store2, __global u * store3,
    __global u * store4, __global u * store5,
    __global u * store6, __global u * store7)
{
...
__kernel __attribute__((vec_type_hint(u))) WORKGROUPSIZE void search2(
    const uint state0, const uint state1, const uint state2, const uint state3,
    const uint state4, const uint state5, const uint state6, const uint state7,
    const uint base,
    __global uint * output,
    ...
    __global u * store6, __global u * store7)
{
  ...
  // hash the hash now
  work[0]=state0+store0[index];
  work[1]=state1+store1[index];
  work[2]=state2+store2[index];
  work[3]=state3+store3[index];
  work[4]=state4+store4[index];
  work[5]=state5+store5[index];
  work[6]=state6+store6[index];
  work[7]=state7+store7[index];
  work[8]=0x80000000;
  work[9]=0x00000000;
  work[10]=0x00000000;
  work[11]=0x00000000;
