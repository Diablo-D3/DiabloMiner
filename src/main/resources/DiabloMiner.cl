...
#define rotr(x, n) amd_bitalign((u)x, (u)x, (u)n)
#else
#define rotr(x, n) rotate((u)x, (u)(32 - n))
#endif
...
__kernel __attribute__((vec_type_hint(u))) void search(
    ...
    __global uint * output)
{
  ...
  for(it = 0; it != LOOPS; it++) {
    ...
    #ifdef VECTORS
    if(H.x == 0)
      output[nonce.x & 0xFF] = nonce.x;

    if(H.y == 0)
      output[nonce.y & 0xFF] = nonce.y;
    #else
    if(H == 0)
      output[nonce & 0xFF] = nonce;
    #endif
#ifdef DOLOOPS
  }
#endif
